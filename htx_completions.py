...
class TagCompletions(sublime_plugin.EventListener):
    def on_query_completions(self, view, prefix, locations):
        ...
        if ch != '<':
            ch2 = view.substr(sublime.Region(pt +1, pt + 2)).strip();
            if ch2 == '$':
                return ([
                        ("$z:get\tTag", "\$z:get[${1:request}.$2]"),
                        ("$z:value\tTag", "\$z:value[$1.$2]"),
        ...
        return ([
                ...
                ("z:set\tTag", "z:set scope=\"${1:request}\" name=\"$2\">$0</z:set>"),
                ("z:unset\tTag", "z:unset scope=\"${1:request}\" name=\"$2\">"),
                ("z:img (global image)\tTag", "z:img picture=\"${1}\">"),
                ("z:img (erp image)\tTag", "z:img path=\$z:value[support.erpfilepath] dir=\"${1:Images}\" name=$2 defpicture=\"${3:missing.picture}\" returnsrc=\"${4:false}\" location=\"config\">"),
                ("z:esc\tTag", "z:esc mode=\"${1:html}\" value=${2}>"),
                ("z:write\tTag", "z:write value=${1}>"),
                ("z:include module\tTag", "z:include module=\"$1\">"),
                ...
                ("z:subst\tTag", "z:subst alias=\"${1}\" use=\"${2}\">"),
                ...
                ("z:ve (Visual Editing)\tTag", "z:ve load=\"${1:ShowWindow}\" guid=${2} itemselected=${3} type=\"${4}\">"),
